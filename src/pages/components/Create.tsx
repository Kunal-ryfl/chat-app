...
const Create = () => {
  ...
  const handleFileSelected = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setfilePath(
      `https://kdrzjjunjsbfuglvmnmk.supabase.co/storage/v1/object/public/posts/${filename}`
    );
  ...
  return (
    <form
      className="  z-10 flex items-center   w-full  max-w-2xl   mx-auto  px-2 py-1  md:py-4 "
      onSubmit={(e) => {
        e.preventDefault();
    ...
    >
      <Image
        src={useSessionData?.user.image || "/img"}
        height={70}
        width={65}
        alt=""
      ...
      <input
        ...
        className="   w-full text-slate-400 text-sm md:text-base font-light bg-transparent outline-none  px-4 py-1   "
        onChange={(e) => setPost(e.target.value)}
      />

      {useSessionData && (
        <label htmlFor="img">
          <FiImage className=" cursor-pointer text-base  mr-2" />
          <input
            type="file"
            id="img"
      ...
      <button className="    font-bold   text-base   text-sky-500    ">
        Post
      </button>
    </form>
  );
